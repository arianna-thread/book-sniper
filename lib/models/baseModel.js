  ...
  module.exports = function(db) {
    ...
    return {
      ...
      _replace: function(collectionName, item) {
        return db.getConnection().then(function(dbInstance) {
          var deferred;
          deferred = Q.defer();
          dbInstance.collection(collectionName, function(err, collection) {
            if (err) {
              deferred.reject(err);
              return;
            }
            return collection.save(item, function(err, result) {
              if (err) {
                deferred.reject(err);
                return;
              }
