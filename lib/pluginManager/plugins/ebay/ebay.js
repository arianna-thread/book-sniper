...
var uriParser = function(uri) {
        ...
        if (uriArray) return uriArray[uriArray.length - 1].match(/\d+/);
    };
...
var getByKey = function(idResource, idType) {
        ...
        rest.get(lookupEbay, {
            ...
            Success ') {
                if (idType === '
            ISBN ') defer.reject(pluginError.factory('
            invalidISBN '));
                else defer.reject(pluginError.factory('
            invalidURI '));
                return;
            }
...
exports['
            raiseHand '] = function(uri, cb) {
    if (uri.match('
            http: //product.half.ebay')) {
        cb(true, 'ebay');
    } else {
        cb(false);
    }
